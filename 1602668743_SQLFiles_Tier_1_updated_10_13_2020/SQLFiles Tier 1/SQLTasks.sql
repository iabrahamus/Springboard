...
FROM Members
WHERE EXTRACT(MONTH FROM joindate) = 09;
...
SELECT CONCAT( S.firstname, ', ', S.firstname ) AS member, 
       CONCAT( F.surname, ', ', F.firstname ) AS recommendedby
FROM Members F
INNER JOIN Members AS S ON F.memid = S.recommendedby
WHERE S.recommendedby !=0
ORDER BY member;

/* Q12: Find the facilities with their usage by member, but not guests */

SELECT F.name AS Facility, CONCAT( M.surname, ', ', M.firstname ) AS Member, COUNT( F.name ) AS Fac_Usage
...
SELECT F.name AS Facility, EXTRACT(
MONTH FROM B.starttime ) AS
MONTH , COUNT( F.name ) AS Fac_Usage
FROM Facilities AS F
INNER JOIN Bookings AS B ON F.facid = B.facid
INNER JOIN Members AS M ON B.memid = M.memid
WHERE M.memid !=0
